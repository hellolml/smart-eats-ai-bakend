...
- 当 `images` 非空时：对每张图片（或其压缩副本）调用 LLM 多模态能力（视觉理解）进行内容识别：
  1. **文字提取**：识别图片中的所有文字内容（标题、正文、地点名称、价格、时间等）。
  2. **结构理解**：理解图片的排版结构（如行程表格、地点列表、地图标注等），按逻辑顺序组织提取的文字。
  3. **地点识别**：在提取的文字中识别地点名称、地址、推荐理由等关键信息。
  4. 将识别结果作为文本内容，等同于 `raw_texts`，进入步骤 3（识别地点提及）。
