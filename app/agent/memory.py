...
_STORE = InMemoryStore() if InMemoryStore else None
...
class _InMemorySaverHistoryCache(_HistoryCache):
    def __init__(self, saver: Any) -> None:
        self._saver = saver
        if not hasattr(self._saver, "_history_cache"):
            self._saver._history_cache = OrderedDict()
        self._cache = self._saver._history_cache
...
def create_history_cache() -> _HistoryCache:
    if InMemorySaver:
        return _InMemorySaverHistoryCache(InMemorySaver())
...
def store_memory(user_id: str | None, text: str) -> None:
    if not _STORE or not user_id:
        return
    _STORE.put((user_id, "memories"), text[:32], {"text": text}, index=False)


def search_memories(user_id: str | None, query: str, limit: int = 3) -> list[str]:
    if not _STORE or not user_id:
        return []
    results = _STORE.search((user_id, "memories"), query=query, limit=limit)
    return [item.value.get("text", "") for item in results]
