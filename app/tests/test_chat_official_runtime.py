...
@pytest.mark.asyncio
async def test_chat_stream_official_runtime_smoke(client, monkeypatch):
            ...
            "tool_calls": [
                {
                    ...
                    "args": {
                        "recommendations": [
                            {"type": "note", "title": "官方模式测试通过", "reason": "official_runtime"}
                        ],
                        "followups": [],
                        "warnings": [],
                    },
    ...
    async with client.stream(
        ...
        async for line in response.aiter_lines():
            ...
            if line.startswith("data:") and current_event == "final":
                ...
                assert recs and recs[0].get("title") == "官方模式测试通过"
                got_final = True
                break
