...
const FOOD_NAMES = [
    ...
    'Chicken', 'Taco', 'Cake', 'Cookie', 'Bread', 'Fries', 'Pasta', 'Curry', 'Steak', 'Rice'
];
const BlindBox: React.FC = () => {
    ...
    const triggerAnimation = useCallback(() => {
        ...
        //4.结果展示阶段（2.8s后）
        setTimeout(() => {
            const index = Math.floor(Math.random() * FOOD_NAMES.length);
            setResultIndex(index);
            setStatus('result');
            setParticles([]);
            isAnimating.current = false;
        }, 2800);
    }, [generateParticles, generateExplosionAndConfetti]);
    ...
    return (
        <div className="h-full flex flex-col items-center justify-center relative overflow-hidden no-scrollbar bg-[#FFF9F2]">
            ...
            <header className="fixed top-0 left-0 right-0 h-16 bg-white/90 backdrop-blur-md border-purple-50 flex items-center px-4 z-50 shadow-sm">
                <button
                    onClick={() => navigate('/')}
                    className="p-2 hover:bg-purple-50 rounder-xl text-gray-600 transition-colors active:scale-90">
                    <ChevronLeft size={24} />
                </button>
                <h1 className="flex-1 text-center mr-10 font-bold text-gray-800">
                    美食盲盒
                </h1>
            </header>
            ...
            <div className="absolute inset-0 pointer-events-none z-40">
                <AnimatePresence>
                    {(status === 'exploding' || status === 'result') &&
                        confetti.map((c) => (
                            <motion.div
                                key={c.id}
            ...
            <div className="text-center mb-12 z-10 mt-16">
                ...
                <p className="text-gary-400 text-sm mt-2">
                    美味汇聚，纠结症的终极克星
                </p>
            </div>
            ...
            <div className="relative w-80 h-80 flex items-center justify-center">
                ...
                <motion.div
                    animate={{
                        scale:
                            status === 'expanding' ? 1.6 : status === 'exploding' ? 2.2 : 1,
                ...
                >
                    ...
                    {status === 'idle' && (
                        <p className="mt-8 text-center text-purple-500 font-bold animate-pulse text-sm">
                            点击开启盲盒
                        </p>
                    )}
                    {status === 'gathering' || status === 'expanding'? (
                        <p className="mt-8 text-center text-purple-500 font-bold text-sm">
                            盲盒开启中...
                        </p>
                    ): null}
                </motion.div>
